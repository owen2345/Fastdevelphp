...
**/
class FD_FormValidator
{
	var $validator_array;
    var $error_hash;
	var $custom_validators;
	
	function FD_FormValidator()
	{
	...
	function isValid($Model)
	{
		...
		{
			if(!$this->ValidateObject($val_obj, $Model, $error_string))
			{
				$bret = false;
		...
		if(true == $bret && count($this->custom_validators) > 0)
		{
			...
			{
				if(false == $custom_val->DoValidate($Model, $this->error_hash))
				{
					$bret = false;
				}
			}
	...
	function ValidateObject($validatorobj, $Model,&$error_string)
	{
		$bret = true;
        ...
        $attrName = $validatorobj->variable_name;
        $class = new ReflectionClass(get_class($Model));
        //if(isset($formvariables[$validatorobj->variable_name]))
        if($class->hasProperty($attrName))
            $input_value = $Model->$attrName;
	...
	function ValidateCommand($command,$command_value,$input_value,&$default_error_message,$variable_name, $Model)
	{
        $class = new ReflectionClass(get_class($Model));
		$bret=true;
		switch($command)
		 ...
		 case "eqelmnt":
						{
                            ...
                            if($class->hasProperty($command_value) && strcmp($input_value, $Model->$command_value)==0 )
							{
								$bret=true;
							}
							else
