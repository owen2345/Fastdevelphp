...
{	
    ...
    */    
	function createGroupRadios($attrsElement = array(), $array_values = array(), $radio_check_value = "")
	{
        ...
        foreach($array_values as $key => $val)
		{
			...
			$options .= "<li><label><input type='radio' ". $this->applyAttrs($attrsElement). " value='".$key."' $sel  /> <span>  ".$val." </span></label></li>";
		}
		return $options. "</ul>";
	}
    ...
    */
    ...
    {
        return $this->uploadFileByTmpname(isset($_FILES[$inputname])?$_FILES[$inputname]['name']:'', isset($_FILES[$inputname])?$_FILES[$inputname]['tmp_name']:'', $dirSave, $support_types, $maximum);
    }
    ...
    */
    function uploadFileByTmpname($filename, $tmpname, $dirSave, $support_types = null, $maximum = null)
    {
        ...
        $res = array("error"=>true, "file"=>"", "error_type" => "no_file", "msg"=>"File not uploaded.");
        if($tmpname) 
        {
    ...
    */
    ...
    /**
     * search and return all directories from this $path
     */
