...
class FD_Session
{
    ...
    {
        $_SESSION["UID_SESSION"]=$User->id_user;
        $_SESSION['UROLE_SESSION'] = $User->role_user;
    ...
    */
	...
	{	   
        $User = $this->Connection->DB->get_object('user', "email_user='$uName'AND pass_user='$uPass'");
        if($User)
		{
    ...
    */
	function check($routeIndex="", $publicNameFunctions = array())
	{
		...
		elseif($routeIndex)
        {
            $this->add_data(array("after_login"=>$_GET['url_fastdevel']));
    ...
    /**
     * check the user role
     * if role is not the same, the user will redirect to $routeIndex
    **/
    function checkRole($routeIndex = "", $role)
    {
        if($_SESSION["UROLE_SESSION"] != $role)
        {
            header("Location: ".ROOT_PATH.$routeIndex);
            exit();
        }
     ...
     */
	function logout($routeIndex = "", $redirect=true)
	{
        session_destroy();
